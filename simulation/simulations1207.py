...
def simulation_correlated_1(
    ...
    adata_sim.obs["shift"] = f"Alpha {alpha}"
...
def run_alpha_shift(
    ...
    seed: int = 0,
    out_folder: str = "figures_alpha_shift",
    adata_folder: str = "newadata",
    alpha_vec: np.ndarray = np.array([0.0]), # slope of the linear shift,
):
    """
    ...
    for alpha in alpha_vec:
        ...
        adata_sim.write(f"{adata_folder}/adata_shift_alpha_{alpha}.h5ad")
        _LOGGER.info(f"Adata saved: {adata_folder}/adata_shift_alpha_{alpha}.h5ad")
        ...
        # row 1 colors (latent basis)
        colors_row1 = ["latent_t"]
        # # row 2 colors (norm umap)
        colors_row2 = ["latent_t"]
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(out_folder, f"combined_embeddings_{alpha}.png"),
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
        _LOGGER.info(f"Plot saved: {out_folder}/combined_embeddings_{alpha}.png")
    
_LOGGER = logging.getLogger(__name__)

if __name__ == "__main__":
    alpha_vec = [0.0, 0.01, 0.05, 0.1]
    run_alpha_shift(
        n_samples=1000,
        n_genes=50,
        sigma=0.05,
