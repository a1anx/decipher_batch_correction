...
def simulation_correlated_2(
    ...
    #latent_z_sampled = np.random.normal(latent_z, sigma)
        # Apply a global shift (delta) to the latent space. Accepts a scalar or a sequence matching latent dimensions.
    if np.isscalar(latent_shift):
        shift = np.full(latent_z.shape[1], float(latent_shift))
    else:
        shift = np.asarray(latent_shift, dtype=float)
        if shift.shape[0] != latent_z.shape[1]:
            raise ValueError(f"latent_shift must be a scalar or a sequence of length {latent_z.shape[1]}")
    latent_z_shifted = latent_z + shift

    latent_z_sampled = np.random.normal(latent_z_shifted, sigma)
    ...
    k_means = KMeans(k_clusters)
    #k_means.fit(latent_z)
    k_means.fit(latent_z_shifted)

    adata_sim.obs["cluster_latent"] = k_means.labels_

    # for i in range(latent_z.shape[1]):
    #     adata_sim.obs[f"latent_z{i}"] = latent_z[:, i]
    for i in range(latent_z_shifted.shape[1]):
        adata_sim.obs[f"latent_z{i}"] = latent_z_shifted[:, i]
    adata_sim.obsm["latent"] = latent_z_sampled
    # latent_names = [f"latent_z{i}" for i in range(latent_z.shape[1])]
    latent_names = [f"latent_z{i}" for i in range(latent_z_shifted.shape[1])]
    adata_sim.uns["latent_z_names"] = latent_names
...
def repeated_benchmark(n_repeats=10, seed=0):
    ...
    for i in range(n_repeats):
        seed = seed_init + i
        adata_sim0 = simulation_correlated_2(
            n_samples=500,
            n_genes=50,
            seed=seed + i,
            sigma=0.03,
        ...
        adata_sim1 = simulation_correlated_2(
            n_samples=500,
            n_genes=50,
            seed=seed + i,
            sigma=0.03,
        ...
        adata_sim = sc.concat([adata_sim0, adata_sim1], label="dataset_id", keys=["no_shift", "shift_0.5"])
...
if __name__ == "__main__":
    ...
    metrics = []
    for seed in range(5):
        for hole_density in [0.000, 0.025, 0.050, 0.075, 0.100]:
            adata_sim0 = simulation_correlated_2(
                n_samples=1000,
                n_genes=50,
                seed=seed,
