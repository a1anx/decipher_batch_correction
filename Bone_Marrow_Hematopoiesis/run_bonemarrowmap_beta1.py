"""
Run batch-corrected Decipher with beta=1.0 - Local Machine Version

This script uses beta=1.0 (instead of 0.1) to increase KL divergence weight.
Higher beta forces more structured latent spaces, which may help preserve V space.
...
- Outputs to Beta_1.0_Training/ folder
"""
...
# Parse arguments
parser = argparse.ArgumentParser(description='Train batch-corrected Decipher with beta=1.0')
...
output_dir = "Beta_1.0_Training"
os.makedirs(output_dir, exist_ok=True)
output_file = f"bonemarrowmap_{args.subset}_{n_epochs}epochs_beta1.h5ad"

print("=" * 80)
print(f"BATCH-CORRECTED DECIPHER (beta=1.0) - {subset_desc}")
print("=" * 80)
...
# Configure for subset size with beta=1.0
config = DecipherBatchCorrectedConfig(
    ...
    # Training parameters (beta=1.0 is the key change)
...
print(f"  Using beta={config.beta} (higher regularization)")
...
ax.set_title(f'Training and Validation Loss - beta=1.0 ({args.subset.upper()} subset)', fontsize=14, fontweight='bold')
ax.legend(fontsize=11)
ax.grid(True, alpha=0.3)
plt.tight_layout()

loss_file = f'bonemarrowmap_{args.subset}_{n_epochs}epochs_beta1_loss_curves.png'
...
print("\n" + "=" * 80)
print("TRAINING SUMMARY (beta=1.0)")
print("=" * 80)
...
print("\n" + "=" * 80)
print("OUTPUTS SAVED TO Beta_1.0_Training/")
print("=" * 80)

print(f"\n1. {output_file}")
print(f"   - AnnData object with beta=1.0 batch-corrected embeddings")
print(f"   - Contains:")
...
print("\n" + "=" * 80)
print("✓ TRAINING COMPLETE (beta=1.0)!")
print("=" * 80)
