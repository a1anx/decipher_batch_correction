"""
Run batch-corrected Decipher with beta=1.0 AND 2 attention heads - Local Machine Version

This script combines two successful modifications:
- beta=1.0 (stronger KL regularization for structured latent spaces)
- n_attention_heads=2 (reduced attention complexity)

Hypothesis: Combining stronger regularization with simpler attention may give
the best balance of batch correction and V-space preservation.

Differences from regular script:
- beta=1.0 (instead of 0.1)
...
- Outputs to Beta_1.0_AttentionHeads_2_Training/ folder
"""
...
# Parse arguments
parser = argparse.ArgumentParser(description='Train batch-corrected Decipher with beta=1.0 and 2 attention heads')
...
output_dir = "Beta_1.0_AttentionHeads_2_Training"
os.makedirs(output_dir, exist_ok=True)
output_file = f"bonemarrowmap_{args.subset}_{n_epochs}epochs_beta1_attnheads2.h5ad"

print("=" * 80)
print(f"BATCH-CORRECTED DECIPHER (beta=1.0 + 2 Attention Heads) - {subset_desc}")
print("=" * 80)
...
# Configure for subset size with beta=1.0 AND 2 attention heads
config = DecipherBatchCorrectedConfig(
    # Latent dimensions
    dim_z=10,                       # Match original Decipher
    dim_v=2,                        # Match original Decipher (for trajectory)

    # Batch correction parameters (combining two successful modifications)
...
ax.set_title(f'Training and Validation Loss - beta=1.0 + 2 Heads ({args.subset.upper()} subset)', fontsize=14, fontweight='bold')
ax.legend(fontsize=11)
ax.grid(True, alpha=0.3)
plt.tight_layout()

loss_file = f'bonemarrowmap_{args.subset}_{n_epochs}epochs_beta1_attnheads2_loss_curves.png'
...
print("\n" + "=" * 80)
print("TRAINING SUMMARY (beta=1.0 + 2 Attention Heads)")
print("=" * 80)
...
print("\n" + "=" * 80)
print("OUTPUTS SAVED TO Beta_1.0_AttentionHeads_2_Training/")
print("=" * 80)

print(f"\n1. {output_file}")
print(f"   - AnnData object with beta=1.0 + 2 heads batch-corrected embeddings")
print(f"   - Contains:")
...
print("\n" + "=" * 80)
print("✓ TRAINING COMPLETE (beta=1.0 + 2 Attention Heads)!")
print("=" * 80)
