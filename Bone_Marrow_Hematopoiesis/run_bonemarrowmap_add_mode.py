"""
Run batch-corrected Decipher with ADD combination mode - Local Machine Version

This script uses combination_mode="add" with batch_emb_dim=10 to match dim_z.
This should preserve V space structure better by reducing decoder input complexity.

Differences from regular script:
- batch_emb_dim=10 (instead of 64)
- combination_mode="add" (instead of "concat")
- Outputs to ComboMode_Add_Training/ folder
"""
...
# Parse arguments
parser = argparse.ArgumentParser(description='Train batch-corrected Decipher with ADD mode')
...
output_dir = "ComboMode_Add_Training"
os.makedirs(output_dir, exist_ok=True)
output_file = f"bonemarrowmap_{args.subset}_{n_epochs}epochs_add_mode.h5ad"

print("=" * 80)
print(f"BATCH-CORRECTED DECIPHER (ADD MODE) - {subset_desc}")
print("=" * 80)
...
# Configure for subset size with ADD mode
config = DecipherBatchCorrectedConfig(
    # Latent dimensions
    dim_z=10,                       # Match original Decipher
    dim_v=2,                        # Match original Decipher (for trajectory)

    # Batch correction parameters (ADD MODE)
    n_batches=None,                 # Will be set from data
    batch_emb_dim=10,               # Match dim_z for ADD mode (reduced from 64)
    decoder_hidden_dims=[],         # Single linear layer (like regular Decipher)
    n_attention_heads=2,            # Multi-head attention (reduced to 2 for divisibility)
    combination_mode="add",         # ADD z and batch effect (changed from concat)
...
print(f"    - Batch embedding dimension: {config.batch_emb_dim} (matches dim_z)")
print(f"    - Decoder hidden layers: {config.decoder_hidden_dims}")
print(f"    - Attention heads: {config.n_attention_heads}")
print(f"    - Combination mode: {config.combination_mode} (ADD)")
print(f"  Dataset:")
...
print(f"  Using ADD combination mode (batch_emb_dim={config.batch_emb_dim})")
...
ax.set_title(f'Training and Validation Loss - ADD Mode ({args.subset.upper()} subset)', fontsize=14, fontweight='bold')
ax.legend(fontsize=11)
ax.grid(True, alpha=0.3)
plt.tight_layout()

loss_file = f'bonemarrowmap_{args.subset}_{n_epochs}epochs_add_mode_loss_curves.png'
...
print("\n" + "=" * 80)
print("TRAINING SUMMARY (ADD MODE)")
print("=" * 80)
...
print("\n" + "=" * 80)
print("OUTPUTS SAVED TO ComboMode_Add_Training/")
print("=" * 80)

print(f"\n1. {output_file}")
print(f"   - AnnData object with ADD mode batch-corrected embeddings")
print(f"   - Contains:")
...
print("\n" + "=" * 80)
print("✓ TRAINING COMPLETE (ADD MODE)!")
print("=" * 80)
