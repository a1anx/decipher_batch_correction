...
adata_regular = sc.read_h5ad(os.path.join(base_dir, "Models", "erythroid_regular_decipher.h5ad"))
adata_bc_beta1 = sc.read_h5ad(os.path.join(base_dir, "Models", "erythroid_batch_corrected_beta1.0_heads2.h5ad"))
adata_bc_beta01 = sc.read_h5ad(os.path.join(base_dir, "Models", "erythroid_batch_corrected_beta0.1_heads4.h5ad"))
...
models = [
    ('Regular Decipher\n(Beta=0.1)', adata_regular, 'decipher_v', 'Regular Decipher'),
    ('Batch-Corrected\nBeta=1.0, 2 heads', adata_bc_beta1, 'X_decipher_v', 'BC Beta=1.0 (2 heads)'),
    ('Batch-Corrected\nBeta=0.1, 4 heads', adata_bc_beta01, 'X_decipher_v', 'BC Beta=0.1 (4 heads)')
]

# Row 1: V-space colored by pseudotime
for col, (title, adata, emb_key, df_key) in enumerate(models):
...
for col, (title, adata, emb_key, df_key) in enumerate(models):
    ...
    ax.plot(x_line, p(x_line), "r--", alpha=0.8, linewidth=2,
            label=f'Linear fit (ρ={metrics_df.loc[df_key, "Spearman (V1 vs pseudotime)"]:.3f})')
...
plt.savefig(os.path.join(base_dir, 'Visualizations', 'pseudotime_v_space_analysis.png'), dpi=150, bbox_inches='tight')
