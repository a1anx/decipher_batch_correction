"""
Run batch-corrected Decipher with batch_emb_dim=16 - Local Machine Version

This script uses CONCAT combination mode with reduced batch_emb_dim=16 (instead of 64).
This should preserve V space structure better by reducing complexity while keeping stability.

Differences from regular script:
- batch_emb_dim=16 (instead of 64)
- combination_mode="concat" (kept for stability)
- Outputs to BatchEmbdDim_16_Training/ folder
"""
...
# Parse arguments
parser = argparse.ArgumentParser(description='Train batch-corrected Decipher with batch_emb_dim=16')
...
output_dir = "BatchEmbdDim_16_Training"
os.makedirs(output_dir, exist_ok=True)
output_file = f"bonemarrowmap_{args.subset}_{n_epochs}epochs_embdim16.h5ad"

print("=" * 80)
print(f"BATCH-CORRECTED DECIPHER (batch_emb_dim=16) - {subset_desc}")
print("=" * 80)
...
# Configure for subset size with batch_emb_dim=16
config = DecipherBatchCorrectedConfig(
    # Latent dimensions
    dim_z=10,                       # Match original Decipher
    dim_v=2,                        # Match original Decipher (for trajectory)

    # Batch correction parameters (batch_emb_dim=16)
    n_batches=None,                 # Will be set from data
    batch_emb_dim=16,               # Reduced from 64, kept for moderate complexity
    decoder_hidden_dims=[],         # Single linear layer (like regular Decipher)
    n_attention_heads=4,            # Multi-head attention (16 is divisible by 4)
...
print(f"    - Batch embedding dimension: {config.batch_emb_dim} (reduced from 64)")
...
print(f"  Using CONCAT mode with batch_emb_dim={config.batch_emb_dim}")
...
ax.set_title(f'Training and Validation Loss - batch_emb_dim=16 ({args.subset.upper()} subset)', fontsize=14, fontweight='bold')
ax.legend(fontsize=11)
ax.grid(True, alpha=0.3)
plt.tight_layout()

loss_file = f'bonemarrowmap_{args.subset}_{n_epochs}epochs_embdim16_loss_curves.png'
...
print("\n" + "=" * 80)
print("TRAINING SUMMARY (batch_emb_dim=16)")
print("=" * 80)
...
print("\n" + "=" * 80)
print("OUTPUTS SAVED TO BatchEmbdDim_16_Training/")
print("=" * 80)

print(f"\n1. {output_file}")
print(f"   - AnnData object with batch_emb_dim=16 batch-corrected embeddings")
print(f"   - Contains:")
...
print("\n" + "=" * 80)
print("✓ TRAINING COMPLETE (batch_emb_dim=16)!")
print("=" * 80)
