"""
Run batch-corrected Decipher with n_attention_heads=1 - Local Machine Version

This script uses n_attention_heads=1 (simplest possible attention).
Minimal attention complexity may help preserve V space structure.

Differences from regular script:
- n_attention_heads=1 (reduced from 4, simplest attention)
...
- Outputs to AttentionHeads_1_Training/ folder
"""
...
# Parse arguments
parser = argparse.ArgumentParser(description='Train batch-corrected Decipher with 1 attention head')
...
output_dir = "AttentionHeads_1_Training"
os.makedirs(output_dir, exist_ok=True)
output_file = f"bonemarrowmap_{args.subset}_{n_epochs}epochs_attnheads1.h5ad"

print("=" * 80)
print(f"BATCH-CORRECTED DECIPHER (1 Attention Head) - {subset_desc}")
print("=" * 80)
...
config = DecipherBatchCorrectedConfig(
    ...
    n_attention_heads=1,            # Minimum: single attention head
...
print(f"    - Attention heads: {config.n_attention_heads} (MINIMUM - simplest attention)")
...
print(f"  Using CONCAT mode with {config.n_attention_heads} attention head (simplest)")
...
ax.set_title(f'Training and Validation Loss - 1 Attention Head ({args.subset.upper()} subset)', fontsize=14, fontweight='bold')
ax.legend(fontsize=11)
ax.grid(True, alpha=0.3)
plt.tight_layout()

loss_file = f'bonemarrowmap_{args.subset}_{n_epochs}epochs_attnheads1_loss_curves.png'
...
print("\n" + "=" * 80)
print("TRAINING SUMMARY (1 Attention Head)")
print("=" * 80)
...
print("\n" + "=" * 80)
print("OUTPUTS SAVED TO AttentionHeads_1_Training/")
print("=" * 80)

print(f"\n1. {output_file}")
print(f"   - AnnData object with 1 attention head batch-corrected embeddings")
print(f"   - Contains:")
...
print("\n" + "=" * 80)
print("✓ TRAINING COMPLETE (1 Attention Head)!")
print("=" * 80)
