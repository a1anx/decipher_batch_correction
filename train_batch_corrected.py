"""
Training script for batch-corrected Decipher model.

This script demonstrates how to train the DecipherBatchCorrected model
...
def train_batch_corrected_decipher(
    adata,
    batch_key,
    config=None,
    ...
    """
    Train a batch-corrected Decipher model.
    ...
    config : DecipherBatchCorrectedConfig, optional
    ...
    # Initialize model
    model = DecipherBatchCorrected(config).to(device)
...
def evaluate_batch_correction(model, adata, batch_key, device='cpu'):
    """
    Evaluate the batch correction by examining attention weights.
    ...
    -------
    results : dict
        Dictionary containing evaluation metrics and attention weights
    """
    ...
    with torch.no_grad():
        for genes_batch, batch_indices in data_loader:
            ...
            attn_weights = model.get_batch_attention_weights(genes_batch, batch_indices)
...
if __name__ == "__main__":
    ...
    print(f"  X_decipher_z: {adata.obsm['X_decipher_z'].shape}")
    print(f"  X_decipher_v: {adata.obsm['X_decipher_v'].shape}")
