"""
Direct comparison between Concatenation and Attention approaches on adata_combined_2.h5ad

This script assumes both models have been trained:
- Concatenation: run_concat_on_data_2.py
- Attention: ../simulated-data-analysis/run_batch_corrected_on_data_2.py
...
print("=" * 80)
print("COMPARING CONCATENATION VS ATTENTION ON adata_combined_2.h5ad")
print("=" * 80)
...
concat_file = '../../simulation/archive/old_adata/adata_combined_2_concat.h5ad'
attention_file = '../../simulation/archive/old_adata/adata_combined_2_batch_corrected.h5ad'

if not os.path.exists(concat_file):
    print(f"\n⚠ ERROR: Concatenation results not found!")
    print(f"  Expected: {concat_file}")
    print(f"  Please run: python run_concat_on_data_2.py")
    exit(1)

if not os.path.exists(attention_file):
    print(f"\n⚠ ERROR: Attention results not found!")
    print(f"  Expected: {attention_file}")
    print(f"  Please run: cd ../simulated-data-analysis && python run_batch_corrected_on_data_2.py")
    exit(1)
...
assert all(adata_concat.obs['shift'] == adata_attention.obs['shift']), "Different batch labels!"
...
# Get batch labels
batch_labels = adata_concat.obs['shift']
...
plt.tight_layout()
plt.savefig('comparison_z_space.png', dpi=300, bbox_inches='tight')
print("✓ Saved: comparison_z_space.png")
plt.close()

# ===== Figure 2: V space comparison =====
fig, axes = plt.subplots(2, 3, figsize=(20, 12))

# Row 1: V space by condition
...
plt.tight_layout()
plt.savefig('comparison_v_space.png', dpi=300, bbox_inches='tight')
print("✓ Saved: comparison_v_space.png")
plt.close()
...
if 'batch_attention_strength' in adata_attention.obs:
    ...
    for shift_val in adata_attention.obs['shift'].unique():
        mask = adata_attention.obs['shift'] == shift_val
        mean_attn = adata_attention.obs.loc[mask, 'batch_attention_strength'].mean()
        print(f"    {shift_val}: {mean_attn:.4f}")
...
print("FILES CREATED")
print("=" * 80)
print("  1. comparison_z_space.png")
print("     - Z latent space comparison (3 approaches)")
print("     - Top row: colored by condition")
print("     - Bottom row: colored by pseudotime")
print("\n  2. comparison_v_space.png")
