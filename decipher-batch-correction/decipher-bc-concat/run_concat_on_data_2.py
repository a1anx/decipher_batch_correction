"""
Run concatenation-based batch-corrected Decipher on adata_combined_2.h5ad

This script:
1. Loads the data
2. Uses 'shift' column as batch information (4 batches)
...
print("=" * 70)
print("CONCATENATION-BASED BATCH CORRECTION - Training on adata_combined_2.h5ad")
print("=" * 70)
...
# Step 1: Load data
print("\n[1/5] Loading data...")
adata = sc.read_h5ad('../../simulation/archive/old_adata/adata_combined_2.h5ad')
print(f"✓ Loaded data: {adata.shape[0]} cells, {adata.shape[1]} genes")
print(f"  Batch column: 'shift' with {len(adata.obs['shift'].unique())} conditions")
print(f"  Conditions: {list(adata.obs['shift'].unique())}")
...
config.initialize_from_adata(adata, batch_key='shift')
...
results = evaluate_batch_correction_concat(model, adata, batch_key='shift', device='cpu')
...
# Step 5: Save results
print("\n[5/5] Saving results...")
output_file = '../../simulation/archive/old_adata/adata_combined_2_concat.h5ad'
...
print("   python visualize_concat_results.py")

print("\n2. Compare with attention approach:")
print("   python compare_concat_vs_attention.py")
