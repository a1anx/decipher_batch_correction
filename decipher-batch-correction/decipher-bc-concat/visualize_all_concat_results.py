"""
Comprehensive visualization script for concatenation-based batch correction

This script combines:
1. visualize_concat_results.py - General batch correction comparison
2. visualize_concat_v_by_pseudotime.py - V space by pseudotime
...
print("=" * 80)
print("COMPREHENSIVE VISUALIZATION: CONCATENATION-BASED BATCH CORRECTION")
print("=" * 80)
...
print("\n[1/5] Loading data...")
adata = sc.read_h5ad('../../simulation/archive/old_adata/adata_combined_2_concat.h5ad')
...
plt.tight_layout()
plt.savefig('concat_method_comparison.png', dpi=300, bbox_inches='tight')
print("    ✓ Saved: concat_method_comparison.png")
plt.close()
...
sc.pl.embedding(adata, basis='decipher_concat_umap', color='shift',
                show=False, ax=ax6, title='Concat Batch-Corrected - Condition')

plt.tight_layout()
plt.savefig('concat_biological_variables.png', dpi=300, bbox_inches='tight')
print("    ✓ Saved: concat_biological_variables.png")
plt.close()
...
if pseudotime is not None:
    ...
    # Panel 3: Concat V by condition
    ax = axes[1, 0]
    for batch in adata.obs['shift'].unique():
        mask = adata.obs['shift'] == batch
        ax.scatter(v_concat[mask, 0], v_concat[mask, 1],
                  c=batch_colors[batch], label=batch, alpha=0.6, s=10)
    ax.set_title('Concat V - by Condition', fontsize=12, fontweight='bold')
    ax.set_xlabel('V Dimension 1')
    ax.set_ylabel('V Dimension 2')
    ax.legend(title='Condition', fontsize=8)
    ax.grid(True, alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig('concat_v_space_four_panel.png', dpi=300, bbox_inches='tight')
    print("    ✓ Saved: concat_v_space_four_panel.png")
    plt.close()
...
if pseudotime is not None:
    ...
    for batch in adata.obs['shift'].unique():
        ...
        ax_left.scatter(v_original[mask, 0], v_original[mask, 1],
                       c=batch_colors[batch], label=batch, alpha=0.6, s=10)
    ax_left.set_title('Original V - by Condition', fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig('concat_v_original_comparison.png', dpi=300, bbox_inches='tight')
    print("    ✓ Saved: concat_v_original_comparison.png")
    plt.close()
    ...
    for batch in adata.obs['shift'].unique():
        ...
        ax_left.scatter(v_concat[mask, 0], v_concat[mask, 1],
                       c=batch_colors[batch], label=batch, alpha=0.6, s=10)
    ax_left.set_title('Concat V - by Condition', fontsize=14, fontweight='bold')
    ax_left.set_xlabel('V Dimension 1', fontsize=12)
    ax_left.set_ylabel('V Dimension 2', fontsize=12)
    ax_left.legend(title='Condition', bbox_to_anchor=(1.05, 1), loc='upper left')
    ax_left.grid(True, alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig('concat_v_batch_corrected_comparison.png', dpi=300, bbox_inches='tight')
    print("    ✓ Saved: concat_v_batch_corrected_comparison.png")
    plt.close()
...
print("\nFiles created:")
print("  1. concat_method_comparison.png")
print("     - 3×3 grid comparing different methods")
print("     - All colored by condition (shift)")
print("     - UMAP, Z space, V space across methods")

print("\n  2. concat_biological_variables.png")
print("     - Biological variables (latent_t, branch_id)")
print("     - Variance explained visualization")
print("     - Condition comparison")

if pseudotime is not None:
    print("\n  3. concat_v_space_four_panel.png")
    print("     - Four-panel V space comparison")
    print("     - Top: Original V (condition vs pseudotime)")
    print("     - Bottom: Concat V (condition vs pseudotime)")

    print("\n  4. concat_v_original_comparison.png")
    print("     - Original V: condition vs pseudotime side-by-side")

    print("\n  5. concat_v_batch_corrected_comparison.png")
    print("     - Concat V: condition vs pseudotime side-by-side")
...
print("  - Look for MIXING of colors (batches) in concat plots")
...
print("=" * 80)
print("\nNext step: Compare with attention using compare_concat_vs_attention.py")
